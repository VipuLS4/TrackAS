/*
  # TrackAS Database Schema - Initial Setup
...
CREATE POLICY "Operators can be updated by authenticated users"
  ON operators FOR UPDATE TO authenticated USING (true);
...
CREATE POLICY "Shipment updates can be viewed by authenticated users"
  ON shipment_updates FOR SELECT TO authenticated USING (true);

CREATE POLICY "Shipment updates can be created by authenticated users"
  ON shipment_updates FOR INSERT TO authenticated WITH CHECK (true);
